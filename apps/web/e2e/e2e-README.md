...
A real bug, confirmed and fixed: the original `if: ${{ secrets.GROQ_API_KEY != '' }}` gate looked like a documented, commonly-used pattern, but GitHub's actual workflow parser rejected it outright on the real first push - `Unrecognized named-value: 'secrets'`, a genuine parser restriction against direct `secrets` access in a job-level `if:` condition, not something either testing or documentation review caught in advance. Fixed with the standard, reliable workaround: a small preceding job (`check-groq-key`) checks the secret's presence via a real shell command and exposes it as a job output; the `e2e` job's `if:` condition reads that output (`needs.check-groq-key.outputs.has_key == 'true'`) instead of touching `secrets` directly - `needs.*.outputs` in a job-level `if:` is a genuinely supported pattern, unlike the original attempt.
